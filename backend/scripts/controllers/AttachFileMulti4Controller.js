angular.module('e-homework').controller('AttachFileMulti4Controller', function($scope, $compile, $cookies, $filter, $state, $routeParams, $uibModal, HTTPService, IndexOverlayFactory) {
    ...
    $scope.addFiles = function(){
        ...
        $scope.FileCode.push({'code_th':'', 'code_en' : ''});
    }
    ...
    $scope.edit = function(data){
        ...
        while((cnt < data.AttachFiles.length)){
            $scope.FileName.push({'name_th':'', 'name_en' : ''});
            $scope.addNewFiles();
            console.log(cnt);
            if(data.AttachFiles[cnt].file_language == 'TH'){
                $scope.FileName[index].name_th = data.AttachFiles[cnt].display_name;
                cnt++;
                // index++;
            }
            if(data.AttachFiles[cnt] != undefined && data.AttachFiles[cnt].file_language == 'EN'){
                $scope.FileName[index].name_en = data.AttachFiles[cnt].display_name;
                cnt++;
