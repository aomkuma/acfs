    ...
    class InfographicController extends Controller {
        ...
        public function updateData($request, $response, $args){
            ...
            try{
                ...
                // print_r($files);
                // exit;
                $AttachFileList  =$files['obj']['AttachFileList'];
                /*
                if($files != null){
                    ...
                    foreach($files as $key => $val){
                        ...
                        foreach($val['AttachFileList'] as $k => $v){
                            ...
                            // $cnt_index = str_replace('_', '', $cnt_index);

                            // print_r($FileName);
                            $file_name_th = $FileName[intval($cnt_index)]['name_th'];
                            $file_name_en = $FileName[intval($cnt_index)]['name_en'];

                            $file_code_th = $FileCode[$cnt_index]['code_th'];
                            $file_code_en = $FileCode[$cnt_index]['code_en'];                            
                            // print_r($v);
                            // exit;
                            $f = $v['attachFileTH'];
                            // print_r($f);
                ...
                */
                ...
                for($i = 0; $i < count($FileName); $i++){
                    ...
                    $f = $AttachFileList[$i]['attachFileTH'];
                    // print_r($f);
                    if($f != null){
                        if($f->getClientFilename() != ''){
                    ...
                    $attachfile_order_no++;
                    $f = $f = $AttachFileList[$i]['attachFileTH'];;
                    // print_r($f);
                    if($f != null){
                        if($f->getClientFilename() != ''){
