angular.module('e-homework').controller('QuestionnaireResponseController', function($scope, $compile, $cookies, $filter, $state, $routeParams, $uibModal, HTTPService, IndexOverlayFactory) {
    ...
    $scope.loadQuestionnaireList = function(keyword){
        var params = {
                        'keyword' : keyword
                    };
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('questionnaire/list/page', params).then(function(result){
            console.log(result);
            if(result.data.STATUS == 'OK'){
                $scope.DataList = result.data.DATA.Questionnaire;
    ...
    $scope.updateResponse = function(standardID){
        window.location.href = '#/questionnaire-response/detail/' + standardID;
    ...
    $scope.setQuestionnaire = function(ID){
        window.location.href = '#/questionnaire-response/detail/' + ID;
